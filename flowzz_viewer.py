...
# Preserve current checkbox state if the editor was used before
prev_editor = st.session_state.get("product_editor")
if isinstance(prev_editor, pd.DataFrame) and "Auswahl" in prev_editor.columns:
    prev_selected = prev_editor[prev_editor["Auswahl"]].index
    select_df.loc[select_df.index.isin(prev_selected), "Auswahl"] = True
...
if len(selected_rows) > 3:
    ...
    strain_ids = selected_rows.index.tolist()
    if manual_select:
